<template>
        ...
        <div class="flex flex-col divide-y w-full">
            <div class="flex flex-row ">
                <div class="select-none cursor-pointer hover:bg-gray-50 flex flex-1 items-center p-4"
                    @drop="onDrops($event, id)" @dragover.prevent v-on:click="show = !show">

                    <div class="flex-1 pl-1">
                        <div class="font-medium text-blueGray-700">{{ title }} </div>
    ...
    <Transition>
        ...
        <div class="flex w-full px-4 py-4 bg-gray-50 border-gray-50 shadow m-3" v-if="show">
            <slot></slot>
        </div>
...
export default {
    ...
    methods: {
        onDrops(evt, id) {
            ...
            router.put(route('menu.update.parent', Number(itemID)), this.formMenu, {
                preserveScroll: true,
                preserveState: true,
                onSuccess: () => {
