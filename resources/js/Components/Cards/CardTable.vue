          ...
          <template v-for="( data, index ) in list">
            <tr class="border-b border-gray-200 ">
              ...
              <template v-for="(datax, index) in header">
                ...
                <img :src="datax.baseurl+data[datax.fieldfoto]" @error="$event.target.src=image" class="h-12 w-12 bg-white rounded-full border" alt="...">
                <span class="ml-3 font-bold text-blueGray-600"> {{ data[datax.fieldtext] ? data[datax.fieldtext] : '-' }} </span>
              </th>
                </template>
                <template v-else>
                  <td 
